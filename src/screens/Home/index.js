...
const HomeScreen = ({navigation}) => {
  ...
  const renderItem = ({item}) => (
    <View style={style.card}>
      {item.banner && <Image source={{uri: item.banner}} style={style.image} />}
  ...
  return (
    <>
      ...
      <ScrollView
        ...
        style={{padding: 12, margin: 0, backgroundColor: '#fee4b4'}}>
        {/* Banner */}
    ...
    {i18n.language === "hi" ? "Switch to English" : "हिंदी में बदलें"}
        ...
        <Text style={style.sectionHeading}>
          {isHindi ? 'आज के कार्यक्रम' : "Today's Programs"}
        </Text>
        <View style={style.card}>
          <View>
          ...
          <Text style={style.text}>
            {isHindi ? 'स्थान: पनकी, कानपुर' : 'Location: Panki, Kanpur'}
          </Text>
          <Text style={style.text}>
            {isHindi ? 'समय: शाम 6 बजे' : 'Time: 6 PM'}
          </Text>
          <Text style={style.text}>
            {isHindi ? 'संपर्क: 9876543210' : 'Contact: 9876543210'}
          </Text>
        </View>
